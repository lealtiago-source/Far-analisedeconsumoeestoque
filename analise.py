...
def executar_analise(caminho_dispensacao, caminho_distribuicao, caminho_estoque, caminho_saida):
    try:
        ...
        # Remove linhas sem nome de medicamento
        ...
        def normalizar(nome):
            nome = str(nome).lower()
            nome = nome.replace("comprimido", "").replace("cápsula", "").replace("drágea", "")
        ...
        # Agrupa as quantidades por produto normalizado
        ...
        df_resultado['Previsão Fim do Estoque'] = df_resultado.apply(
            lambda row: (
                datetime.today() + timedelta(days=(row['Quantidade em Estoque'] / row['Dias de Consumo'] * 30))
            ).strftime('%d/%m/%Y') if row['Dias de Consumo'] > 0
            else 'FALTA DESDE ' + datetime.today().strftime('%d/%m/%Y'),
            axis=1
        )
