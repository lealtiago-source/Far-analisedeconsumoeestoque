...
def carregar_dados(arquivo1, arquivo2, arquivo3):
    df_disp = pd.read_excel(arquivo1,
        sheet_name='relatorioDispensacaoPorMedicame',
        usecols=['Data Dispensação', 'Medicamento/Produto', 'Lote', 'Quantidade Dispensada'])

    df_dist = pd.read_excel(arquivo2,
        sheet_name='distribuíções',
        usecols=['Data Distribuição', 'Medicamento/Produto', 'Lote', 'Quantidade distribuída (unidades)'])

    df_estoque = pd.read_excel(arquivo3, sheet_name='estoque')
...
def executar_analise(caminho1, caminho2, caminho3, caminho_saida):
    df_comb, df_estoque = carregar_dados(caminho1, caminho2, caminho3)
